...
describe('twoPhase stats 引用一致性', () => {
  const outDir = path.join(process.cwd(), '.tmp_e2e_twoPhase_stats_ref')
  const tempDir = path.join(process.cwd(), '.tmp_e2e_twoPhase_stats_tmp')
  const cfg = {
  ...
  it('saveImage 接收的 stats.formatCounts 与返回结果中的 formatCounts 引用相同，且累计可观测（twoPhase）', async () => {
