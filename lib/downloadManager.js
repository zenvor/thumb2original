// 门面层：保持对外 API 兼容性，内部委托给拆分后的模块
import { orchestrateDownload } from './scraperOrchestrator.js'
import { orchestrateScraping } from './scraperOrchestrator.js'
...
export async function saveImage(buffer, filePath, imageUrl, stats, imageInfoList = null) {
  // 委托给新的文件管理器
  return await saveImageImpl(buffer, filePath, imageUrl, stats, imageInfoList)
}
...
export async function scrapeUrl(url, browser, config, loadAndScrollPage, extractImageUrls, processUrlsByImageMode) {
  // 委托给新的编排器
  return await orchestrateScraping(url, browser, config, loadAndScrollPage, extractImageUrls, processUrlsByImageMode)
}