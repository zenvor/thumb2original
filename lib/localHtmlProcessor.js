...
async function processSingleHtmlFile(fileInfo, index, totalCount, browser, config, downloadManager) {
  ...
  try {
    ...
    let finalImageUrls = await processImageUrlsByMode(imageUrls, config.imageMode, config)
    ...
    handleFatalError(error, `HTML文件: ${path.basename(htmlFile)}`, logger)
  }
 ...
 * 根据图片模式处理URL列表
 */
async function processImageUrlsByMode(imageUrls, imageMode, config) {
  ...
  if (config?.debug?.logImageUrls) {
    // 中文注释：按需打印所有原图链接，便于排查与核对
    try {
      const header = `原始图片链接（共 ${originalUrls.length} 个）：`
      const body = originalUrls.map(u => ` - ${u}`).join('\n')
      logger.info(`${header}\n${body}`)
    } catch {}
  }
  return originalUrls
