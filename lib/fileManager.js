...
function resolveHashSource(imageUrl) {
  if (!imageUrl || typeof imageUrl !== 'string') {
    return ''
  }

  if (imageUrl.startsWith('data:')) {
    return imageUrl.slice(0, 128)
  }

  try {
    const urlObj = new URL(imageUrl, 'http://placeholder')
    const pathWithQuery = `${urlObj.pathname || ''}${urlObj.search || ''}`
    return pathWithQuery || urlObj.href || imageUrl
  } catch {
    return imageUrl
  }
}
 ...
 * @param {object} headers - HTTP 响应头
 * @param {object} config - 配置对象
 * @param {number} sequenceNumber - 图片序号（从 1 开始）
 * @param {number} totalCount - 总图片数量（用于计算零填充位数）
 * @returns {string} 生成的文件名
 */
export function generateFileName(imageUrl, buffer, headers = {}, config = {}, sequenceNumber = null, totalCount = null) {
  let fileName
  ...
  // 防止文件名冲突：添加URL路径哈希后缀
  if (config?.format?.preventNameCollision) {
    const hashSource = resolveHashSource(imageUrl)
    if (hashSource) {
  ...
  // 添加序号前缀（默认启用，除非明确禁用）
  const enableSequence = config?.format?.enableSequenceNumber !== false
  if (enableSequence && sequenceNumber !== null && totalCount !== null) {
    // 计算零填充位数
    const digits = String(totalCount).length
    const paddedNumber = String(sequenceNumber).padStart(digits, '0')
    ...
    if (lastDotIndex > 0) {
      ...
      fileName = `${paddedNumber}_${baseName}${extension}`
    } else {
      // 没有扩展名的情况（罕见）
      fileName = `${paddedNumber}_${fileName}`
    }
  }
