 ...
 */
export async function saveImage(buffer, filePath, imageUrl, stats, imageInfoList = null, config = undefined) {
  try {
    let finalBuffer = buffer
    let finalFilePath = filePath
    const originalFormat = identifyImageFormat(buffer)
    // 中文注释：全局格式转换开关（默认开启）
    ...
    try {
      ...
      // 中文注释：记录最终保存的图片格式计数（用于输出 PNG 21，JPEG 10 等）
      try {
        const detected = identifyImageFormat(finalBuffer)
        let key = detected && typeof detected === 'string' ? detected.toLowerCase() : ''
        if (!key || key === 'unknown') {
          const ext = (finalFilePath.split('.').pop() || '').toLowerCase()
          key = ext
        }
        if (key) {
          if (!stats.formatCounts) stats.formatCounts = Object.create(null)
