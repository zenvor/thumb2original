    ...
    if (maxHoldBuffers > 0 && heldEntries.length > 0) {
      if (db && taskId) {
        ...
        try {
          ...
          logger.debug(`批量保存 ${imagesToSave.length} 张图片到数据库`, logger.categories.DOWNLOAD)
          ...
          logger.warn(`批量保存内存缓存图片到数据库失败，降级到临时文件: ${error.message}`, logger.categories.DOWNLOAD)
          // 数据库保存失败，降级到临时文件
          for (const e of heldEntries) {
  ...
  for (let i = 0; i < validEntries.length; i += concurrentDownloads) {
    ...
    const tasks = batch.map(async (entry, idx) => {
      ...
      try {
        ...
        // 如果使用数据库，直接从数据库读取
        if (entry.fromDatabase && db && taskId) {
...
async function analyzeImageForTwoPhase(
  ...
  if (db && taskId) {
    try {
      ...
      logger.warn(`保存图片到数据库失败: ${error.message}`, logger.categories.DOWNLOAD)
      // 数据库保存失败，降级到临时文件
      const tempPath = await writeBufferToTemp(entry.buffer, tempDir)
      tempFile = {
        bytesWritten: entry.buffer.length,
        entry: { url: entry.url, tempPath, headers: entry.headers, metadata: entry.metadata, sequenceNumber, totalCount }
      }
    }
  } else {
    // 数据库未启用，使用临时文件或内存缓存
...
async function processTwoPhase(imageUrls, targetDownloadDir, context, downloadedImages) {
  ...
  if (db) {
    ...
    logger.info(`使用临时文件存储图片分析结果 (目录: ${tempDir})`, logger.categories.DOWNLOAD)
  }
...
/**
 * twoPhaseApi 模式：仅分析阶段，保存到数据库（或临时文件），不进行下载。
 * - 复用 twoPhase 的 analyzePhase()
 * - 优先使用数据库存储，数据库未启用时才使用临时文件
 * - 数据库启用时，不创建 .tmp_analysis 目录
 */
async function processTwoPhaseApi(imageUrls, targetDownloadDir, context, downloadedImages) {
  ...
  if (db) {
    ...
    logger.debug(`twoPhaseApi：分析完成，已保存到临时文件。临时文件 created=${tempFilesCreated} bytes=${tempBytesWritten} keep=${validEntries.length}`)
  }
  ...
  return {
    ...
    getImagesWithBuffers: async () => {
      return db ? db.getImagesWithBuffers(taskId) : null
    }
  }
}
