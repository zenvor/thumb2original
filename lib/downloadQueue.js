    ...
    if (maxHoldBuffers > 0 && heldEntries.length > 0) {
      ...
      if (db && taskId) {
        try {
          ...
          logger.warn(`批量保存内存缓存图片到数据库失败: ${error.message}`, logger.categories.DOWNLOAD)
        }
      }
      ...
      try {
        ...
        if (db && taskId) {
          try {
            ...
            logger.warn(`从数据库读取图片失败，回退到临时文件: ${error.message}`, logger.categories.DOWNLOAD)
          }
        }

        // 如果数据库读取失败或未启用，从临时文件读取
        if (!buffer) {
