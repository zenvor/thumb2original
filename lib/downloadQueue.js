...
async function processImage(imageUrl, index, context, config, targetDownloadDir, downloadedImages, stats, aggregateStats, totalAnalyzed, batchAnalysisSamples) {
  ...
  // 保存图片（静默模式）
  const fileName = generateFileName(imageUrl, commonResult.imageData.buffer, commonResult.imageData.headers)
  const filePath = path.join(targetDownloadDir, fileName)
  const saveResult = await saveImageSilent(commonResult.imageData.buffer, filePath, imageUrl, stats, downloadedImages, context.config, commonResult.analysisResult)
...
async function analyzePhase(imageUrls, context, tempDir, aggregateStats) {
  ...
  while (urlsToAnalyze.length > 0 && currentRetry <= maxRetries) {
    ...
    for (let i = 0; i < urlsToAnalyze.length; i += concurrentDownloads) {
      ...
      const batchEnd = Math.min(i + concurrentDownloads, urlsToAnalyze.length)
      ...
      try {
        ...
        logger.progress(currentProgress, urlsToAnalyze.length, `分析批次 ${batchIndex} 完成`, 1)
  ...
  for (let i = 0; i < validEntries.length; i += concurrentDownloads) {
    ...
    const tasks = batch.map(async (entry, idx) => {
      ...
      try {
        ...
        fileName = generateFileName(entry.url, buffer, entry.headers)
        const filePath = path.join(targetDownloadDir, fileName)
        await saveImageSilent(buffer, filePath, entry.url, downloadStats, downloadedImages, config, 
    ...
    while (urlsToProcess.length > 0 && currentRetry <= config.maxRetries) {
      if (currentRetry > 0) {
        logger.warn(
          ...
          logger.categories.DOWNLOAD,
          { currentRetry, maxRetries: config.maxRetries, pending: urlsToProcess.length }
        )
        await delay(config.retryDelayMs)
      }
      ...
      for (let i = 0; i < urlsToProcess.length; i += concurrentDownloads) {
        ...
        const batchIndex = Math.floor(i / concurrentDownloads) + 1
        const batchStart = i + 1
        const batchEnd = Math.min(i + concurrentDownloads, urlsToProcess.length)
        ...
        try {
          ...
          logger.progress(currentProgress, urlsToProcess.length, `批次 ${batchIndex} 完成`, 1)
...
/**
 * twoPhaseApi 模式：仅分析阶段，写入临时文件，不进行下载。
 * - 复用 twoPhase 的 analyzePhase()
 * - 保留临时文件供上层 API 使用（不在此处清理）
 */
async function processTwoPhaseApi(imageUrls, targetDownloadDir, context, downloadedImages) {
  ...
  // 输出最终统计信息（下载阶段被跳过）
  outputFinalStats(aggregateStats)
  logger.debug(`twoPhaseApi：跳过下载阶段，仅分析。临时文件 created=${tempFilesCreated} bytes=${tempBytesWritten} keep=${validEntries.length}`)
