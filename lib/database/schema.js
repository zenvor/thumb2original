...
export function initSchema(db) {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS images (
      ...
      image_mode TEXT DEFAULT 'all',    -- 图片模式：all/original
  ...
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_images_mode ON images(task_id, image_mode);
...
export function migrate(db) {
  ...
  // v1 -> v2: 添加 image_mode 字段
  if (currentVersion < 2) {
    // 检查列是否已存在
    const tableInfo = db.prepare("PRAGMA table_info(images)").all()
    const hasImageMode = tableInfo.some(col => col.name === 'image_mode')

    if (!hasImageMode) {
      db.exec(`ALTER TABLE images ADD COLUMN image_mode TEXT DEFAULT 'all'`)
      db.exec(`CREATE INDEX IF NOT EXISTS idx_images_mode ON images(task_id, image_mode)`)
    }
