...
export class ImageAnalysisDB {
   ...
   * @param {string} imageMode - 图片模式：'all' | 'original'
   * @returns {number} 插入的图片ID
   */
  saveImage(taskId, imageData, imageMode = 'all') {
    this._ensureInitialized()
    ...
    const stmt = this.db.prepare(`
      INSERT INTO images (
        ...
        headers, sequence_number, image_mode, created_at
      )
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `)
    ...
    logger.debug(`图片已保存到数据库: ${imageData.url} (ID: ${result.lastInsertRowid}, mode: ${imageMode}, bufferSize: ${imageData.buffer.length})`)
   ...
   */
  saveImageBatch(taskId, imagesData, imageMode = 'all') {
    ...
    logger.info(`批量保存图片完成: ${imagesData.length} 张 (mode: ${imageMode})`)
   ...
   * @returns {Array} 图片数组
   */
  getImagesByMode(taskId, imageMode, includeBuffer = true) {
    this._ensureInitialized()
    const fields = includeBuffer
      ? '*'
      : 'id, task_id, url, format, width, height, size, headers, sequence_number, image_mode, created_at'

    const stmt = this.db.prepare(`
      SELECT ${fields} FROM images
      WHERE task_id = ? AND image_mode = ?
      ORDER BY sequence_number, id
    `)

    const images = stmt.all(taskId, imageMode)
    return images.map(img => ({
      ...img,
      headers: img.headers ? JSON.parse(img.headers) : null
    }))
