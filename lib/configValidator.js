 ...
 */
function setDatabaseDefaults(config) {
  if (!config.database) config.database = {}

  // 默认路径
  if (!config.database.path) {
    config.database.path = './data/analysis.db'
  }

  // 默认启用 WAL 模式
  if (typeof config.database.enableWAL !== 'boolean') {
    config.database.enableWAL = true
  }

  // 默认保留 24 小时
  const retentionHours = Number(config.database.retentionHours)
  config.database.retentionHours = Number.isFinite(retentionHours) && retentionHours > 0
    ? retentionHours
    : 24

  // 默认启用自动清理
  if (typeof config.database.autoCleanup !== 'boolean') {
    config.database.autoCleanup = true
  }

  // 默认清理间隔 1 小时
  const cleanupInterval = Number(config.database.cleanupInterval)
  config.database.cleanupInterval = Number.isFinite(cleanupInterval) && cleanupInterval > 0
    ? cleanupInterval
    : 3600000
}
