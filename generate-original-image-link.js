...
export function generateOriginalImageLink(thumbnailLink) {
  if (thumbnailLink.includes('https://i.pximg.net')) {
    // 对于网站 "https://i.pximg.net"
    // 去掉缩略图链接中的`c/250x250_80_a2/`
    let originalUrl = thumbnailLink.replace(/c\/250x250_80_a2\//, '')
    ...
    // 返回原图链接
    return originalUrl
  } else if (thumbnailLink.includes('https://i001.imx.to')) {
    // 对于网站 "https://i001.imx.to"
    // 将"/t/"替换为"/i/"
    return thumbnailLink.replace('/t/', '/i/')
  } else if (thumbnailLink.includes('https://i8.vipr.im')) {
    // 对于网站 "https://i8.vipr.im"
    // 将"/th/"替换为"/i/"
    return thumbnailLink.replace('/th/', '/i/')
  } else if (thumbnailLink.includes('https://boobsphoto.name')) {
    const urlRegex = /https:\/\/boobsphoto\.name\/uploads\/posts\/.*\.jpg/g;
    const match = thumbnailLink.match(urlRegex)
    return match[0]
  } else {
    // 如果是其他网站，返回原始链接
